...
def calc_epsilon_phi(date):
    ...
    if True:
        ...
        delta_epsilon = 9.20 * GCMath.cosDeg(omega) + 0.57 * GCMath.cosDeg(2 * l) + 0.10 * GCMath.cosDeg(
            2 * ls) - 0.09 * GCMath.cosDeg(2 * omega)

        delta_phi = (-17.20 * GCMath.sinDeg(omega) - 1.32 * GCMath.sinDeg(2 * l) - 0.23 * GCMath.sinDeg(
            2 * ls) + 0.21 * GCMath.sinDeg(2 * omega)) / 3600
    else:
...
def eclipticalToEquatorialCoords(ecc, date):
    ...
    eqc.rightAscension = GCMath.arcTan2Deg(
        GCMath.sinDeg(ecc.longitude) * GCMath.cosDeg(epsilon) - GCMath.tanDeg(ecc.latitude) * GCMath.sinDeg(epsilon),
        GCMath.cosDeg(ecc.longitude))

    eqc.declination = GCMath.arcSinDeg(
        GCMath.sinDeg(ecc.latitude) * GCMath.cosDeg(epsilon) + GCMath.cosDeg(ecc.latitude) * GCMath.sinDeg(
            epsilon) * GCMath.sinDeg(ecc.longitude))
...
def equatorialToHorizontalCoords(eqc, obs, date):
    ...
    hc.azimut = GCMath.rad2deg(math.atan2(GCMath.sinDeg(h),
                                          GCMath.cosDeg(h) * GCMath.sinDeg(obs.latitude_deg) - GCMath.tanDeg(
                                              eqc.declination) * GCMath.cosDeg(obs.latitude_deg)))

    hc.elevation = GCMath.rad2deg(math.asin(
        GCMath.sinDeg(obs.latitude_deg) * GCMath.sinDeg(eqc.declination) + GCMath.cosDeg(
            obs.latitude_deg) * GCMath.cosDeg(eqc.declination) * GCMath.cosDeg(h)))
